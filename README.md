...
**雨燕输入法谨遵循必要、最小化申请权限，只为输入而存在，纯净、安全、更高效。**
雨燕输入法`离线版`无手写功能，仅使用系统默认为输入法开启的`剪贴板`（剪贴板功能使用）、`设备运动与方向`（屏幕方向变更）、`媒体音控制`（按键音效）权限，不获取网络、存储、位置、辅助功能等其他权限，完全离线不上传云端，输入数据不采集、不记录，不访问任何个人、终端、位置、存储等信息;
雨燕输入法`在线版`包含手写功能，集成`汉王在线识别增值服务`，需通过网络上传手写坐标，实现识别功能。因此，`在线版`在`离线版`的基础上增加`网络权限`，为需要手写的用户提供服务。
用户可按需下载对应版本。
### 基于Rime引擎，但更易上手。
...
## 实现功能：
+ 拼音输入：全拼（九宫格、拼键）、双拼(小鹤、智能ABC、自然码、紫光、微软、搜狗)、乱序17双拼；支持简拼、全拼；
+ 英文输入：智能全键英文输入；
